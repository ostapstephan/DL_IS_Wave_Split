...
def hard_shape( layer ,batch=BATCH_SIZE):
    ...
    shape = [ # encoder: model.encoder.Block.weights_shape
    [
    [[1,1,256],[256]],
    [[1,1,256]],
    ['lol',[batch, 40000, 256]],
...
class Conv_1D_Block(object):
    def __init__(self,input_shape,dilation=1, B = 128,hs= 1):
        self.hyper_params= {
        'N' : 128,
        'L' : 1,
        'B' : 128,
        ...
        self.weights_shape = hard_shape( hs )
    ...
    def getShapesofBlock(self):
        ...
        #LayerNorm1 = tf.keras.layers.BatchNormalization(axis=2,dtype='float32')
        LayerNorm1 = tf.keras.layers.Dropout(rate = .1,dtype='float32')
        LayerNorm1_inp = prelu1.compute_output_shape(prelu1_inp)
        LayerNorm1.build(LayerNorm1_inp)
        self.weights_shape.append(['lol',LayerNorm1_inp]) # append the whole layer
        ...
        #LayerNorm2 = tf.keras.layers.BatchNormalization(axis=2,dtype='float32')
        LayerNorm2 = tf.keras.layers.Dropout(rate = .1,dtype='float32')
        LayerNorm2_inp = prelu2.compute_output_shape(prelu2_inp)
        LayerNorm2.build(LayerNorm2_inp)
        self.weights_shape.append(['lol',LayerNorm2_inp]) # appending the whole layer
    ...
    def get_weights(self):
        ...
        for i in range(len(self.weights_shape)): # weight_shape_list
            print(self.weights_shape[i])
            #import pdb; pdb.set_trace()
            if not self.weights_shape[i][0] == 'lol':
                ...
                # this is done so we can get the weights and call the function later

                #temp = tf.keras.layers.BatchNormalization(axis=2,dtype='float32')
                temp = tf.keras.layers.Dropout(rate = .1,dtype='float32')
    ...
    def section_2(self,input_tensor):
        ...
        LayerNorm1 = tf.nn.dropout(input_tensor,rate= .1 )
        return LayerNorm1
        # weights[2] == keras_layers.get_trainable_weights
    ...
    def section_4(self,input_tensor):
        ...
        LayerNorm2 = tf.nn.dropout(input_tensor,rate= .1 )
        return LayerNorm2
...
class Coder(object):
    def __init__(self,input_shape,start=True):
        ...
        if  start:
            self.Block = Conv_1D_Block(input_shape,hs = 0)
        else:
            self.Block = Conv_1D_Block(input_shape, B = 2,hs = 2)
...
class SeparationStack(object):
    def __init__(self,input_shape, Height,Width,hyper_params = None):
        ...
        self.weights_shape = hard_shape(3)

        shape = 0
        for i in range(Width):
            add = []
            for j in range(Height):
                add.append(Conv_1D_Block(shape,2**j,hs=1))
    ...
    def getShapesofbeginningBlock(self):
        ...
        self.weights_shape.append(['lol',self.input_shape])
    ...
    def forward_pass(self,input_tensor):
        ...
        layernorm_1 = tf.nn.dropout(input_tensor,rate= .1 )
...
def main():
    ...
    train_ds = get_dataset_from_tfrecords(tfrecords_dir='/share/audiobooks/tf_records',batch_size=BATCH_SIZE)
